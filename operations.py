...
from MinCut_Sample import FastCheck
...
def vertex_cut(flow_g, s, t) -> set:
    ...
    :return: 所有分割的点集
    """
    nodes = cuts.minimum_node_cut(flow_g, s, t, flow_func=shortest_augmenting_path)
    s = set()
    for node in nodes:
...
def overlap_partition(g: nx.Graph, vertex_cut: set) -> list:
    ...
    for component in components:
        tmp = component.union(vertex_cut)
        s.append(tmp)
    return s
...
def sparse_certificate_random(g: nx.Graph, k):
    """
    生成g的稀疏等价图，同时生成side-groups
    :param g: 原图
    :return: 稀疏等价图
...
def local_cut(u, v, flow_g: nx.Graph, g: nx.Graph, k):
    ...
    if g.has_edge(u, v) or u == v:
        return None
    if maximum_flow_value(flow_g, u1, v, flow_func=shortest_augmenting_path, cutoff=k) >= k:
        return None
    else:
...
def global_cut(g: nx.Graph, k):
    ...
    for vaidx in range(len(neighbors)):
        for vbidx in range(vaidx + 1, len(neighbors)):
            if g.has_edge(neighbors[vaidx], neighbors[vbidx]): continue
            cut = local_cut(neighbors[vaidx], neighbors[vbidx], flow_sc, sc, k)
            if cut is not None:
                return cut
    return None


def kvcc_enum(g: nx.Graph, k, lmd, budget=1) -> list:
    """
    寻找图中所有的k-vcc
    :param g: 输入的无向图
    :param k: k
    ...
    if cut is None:
        ...
        sub_component = max(sub_components, key=len)
        sub_component_g = nx.Graph(component_g.subgraph(sub_component))
        for u in cut:
...
def kvcc_enum_random(g: nx.Graph, k, lmd, budget=1):
    ...
    if len(g)<budget*lmd:
        return None,None,set(g.nodes())
    if k==2:
        bicc=list(nx.biconnected_components(g))
        component=max(bicc,key=len)
        component_g=nx.Graph(g.subgraph(component))
        return component_g,None,set(component)
    #vcc = []
    g_core = k_core(g, k)
    components = list(nx.connected_components(g_core))
    # print("len component",len(components))
...
def global_cut_optimization(g: nx.Graph, k):
    ...
    dist = single_source_shortest_path_length(g, u)
...
def k_vcc_exact(g: nx.Graph, k, lmd):
    strong_side_vertices(g, k)
    return kvcc_enum(g, k, lmd)


def k_vcc(g: nx.Graph, k, lmd,budget=1):
    # strong_side_vertices(g, k)
    cut=1
    cang=g
    while cut:
        cang,cut,internal=kvcc_enum_random(cang, k,lmd,budget)
    if cang:
        return [list(cang.nodes())],None
    else:
        return [],internal
